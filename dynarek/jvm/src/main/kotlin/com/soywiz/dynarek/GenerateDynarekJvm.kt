...
fun MethodVisitor.visit(expr: DExpr<*>): Unit = when (expr) {
	is DArg<*> -> {
		...
		when (clazz) {
			java.lang.Integer.TYPE.kotlin -> {
				//_visitVarInsn(ILOAD, aindex)

				_visitVarInsn(ALOAD, aindex) // Value is boxed!
				_visitTypeInsn(CHECKCAST, java.lang.Integer::class.java)
				_visitMethodInsn(INVOKEVIRTUAL, "java/lang/Integer", "intValue", "()I", false) // unbox
			}
			else -> {
				//println("clazz: $clazz")
				_visitVarInsn(ALOAD, aindex)
				_visitTypeInsn(CHECKCAST, clazz.java)
			}
		}
	}
	is DBinopInt -> {
	...
	is DFieldAccess<*, *> -> {
		visit(expr.obj)
		_visitTypeInsn(CHECKCAST, expr.clazz.java)
...
fun <T> _generateDynarek(func: DFunction, nargs: Int, interfaceClass: Class<T>): T {
...
actual fun <TRet : Any> DFunction0<TRet>.generateDynarek(): () -> TRet = _generateDynarek(this, 0, Function0::class.java) as (() -> TRet)
actual fun <TRet : Any, T0 : Any> DFunction1<TRet, T0>.generateDynarek(): (T0) -> TRet = _generateDynarek(this, 1, Function1::class.java) as ((T0) -> TRet)
actual fun <TRet : Any, T0 : Any, T1 : Any> DFunction2<TRet, T0, T1>.generateDynarek(): (T0, T1) -> TRet = _generateDynarek(this, 2, Function2::class.java) as ((T0, T1) -> TRet)
