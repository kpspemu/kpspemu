...
open class UtilsBase(emulator: Emulator, name: String, flags: Int, prx: String, prxName: String) : SceModule(emulator, name, flags, prx, prxName) {
	val random = MtRand(0)
	...
	fun sceKernelDcacheWritebackRange(ptr: Int, size: Int): Unit = emulator.dataCache(writeback = true, invalidate = false)
	fun sceKernelDcacheWritebackInvalidateAll(): Unit = emulator.dataCache(writeback = true, invalidate = true)
	fun sceKernelDcacheWritebackInvalidateRange(ptr: Int, size: Int): Unit = emulator.dataCache(ptr, size, writeback = true, invalidate = true)
	fun sceKernelDcacheInvalidateRange(ptr: Int, size: Int): Unit = emulator.dataCache(ptr, size, writeback = false, invalidate = true)
	...
	override fun registerModule() {
		// Rand
		registerFunctionInt("sceKernelUtilsMt19937Init", 0xE860E75E, since = 150, syscall = 0x20BF) { sceKernelUtilsMt19937Init(ptr, int) }
		...
		registerFunctionVoid("sceKernelDcacheWritebackInvalidateAll", 0xB435DEC5, since = 150) { sceKernelDcacheWritebackInvalidateAll() }
		registerFunctionVoid("sceKernelDcacheInvalidateRange", 0xBFA98062, since = 150) { sceKernelDcacheInvalidateRange(int, int) }
		registerFunctionVoid("sceKernelDcacheWritebackRange", 0x3EE30821, since = 150) { sceKernelDcacheWritebackRange(int, int) }
