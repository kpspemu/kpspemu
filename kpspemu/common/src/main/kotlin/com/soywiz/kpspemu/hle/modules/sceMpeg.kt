...
class sceMpeg(emulator: Emulator) : SceModule(emulator, "sceMpeg", 0x00010011, "mpeg_vsh.prx", "sceMpegVsh_library") {
	...
	override fun registerModule() {
		...
		registerFunctionRaw("sceMpegRingbufferQueryPackNum", 0x769BEBB6, since = 150) { sceMpegRingbufferQueryPackNum(it) }
		...
		registerFunctionRaw("sceMpegRingbufferAvailableSize", 0xB5F6DC87, since = 150) { sceMpegRingbufferAvailableSize(it) }
		...
		registerFunctionRaw("sceMpegRingbufferQueryMemSize", 0xD7A29F46, since = 150) { sceMpegRingbufferQueryMemSize(it) }
