...
class sceUmdUser(emulator: Emulator) : SceModule(emulator, "sceUmdUser", 0x40010011, "np9660.prx", "sceNp9660_driver") {
	...
	suspend fun _sceUmdWaitDriveStatTimeout(stat: Int, timeout: Int): Int {
		try {
			...
			withOptTimeout(if (timeout < 0) null else timeout / 1000, "_sceUmdWaitDriveStat") {
	...
	override fun registerModule() {
		...
		registerFunctionSuspendInt("sceUmdWaitDriveStat", 0x8EF08FCE, since = 150, cb = false) { sceUmdWaitDriveStat(int) }
		registerFunctionSuspendInt("sceUmdWaitDriveStatCB", 0x4A9E5E29, since = 150, cb = true) { sceUmdWaitDriveStatCB(int, int) }
		registerFunctionSuspendInt("sceUmdWaitDriveStatWithTimer", 0x56202973, since = 150) { sceUmdWaitDriveStatWithTimer(int, int) }
