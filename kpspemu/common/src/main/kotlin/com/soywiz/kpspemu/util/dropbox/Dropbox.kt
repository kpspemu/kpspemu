...
class Dropbox(val bearer: String, val http: HttpClient = createHttpClient()) {
	...
	suspend fun createFolder(path: String): Unit {
		val rpath = if (path == "/") "" else path
		val result = jsonRawRequest("2/files/create_folder", hashMapOf<String, Any>(
			"path" to rpath,
			"autorename" to false
		))
	...
	suspend fun upload(path: String, content: AsyncStream): Entry {
		val req = rawRequest("2/files/upload", headers = Http.Headers(
...
fun <T : Comparable<T>> ClosedRange<T>.overlapsWith(that: ClosedRange<T>): Boolean = this.start <= that.endInclusive && that.start <= this.endInclusive
