...
object InstructionTable {
	...
	data class ValueMask(val value: Int, val mask: Int)

	fun VM(format: String): ValueMask {
		val counts = mapOf(
	...
	fun ID(name: String, vm: ValueMask, format: String, addressType: Int, instructionType: Int) {
	...
	init {
		...
		ID("beql", VM("010100:rs:rt:imm16"), "%s, %t, %O", ADDR_TYPE_16, INSTR_TYPE_B or INSTR_TYPE_LIKELY);

		// Branch on Greater Equal Zero (And Link) (Likely).
		ID("bgez", VM("000001:rs:00001:imm16"), "%s, %O", ADDR_TYPE_16, INSTR_TYPE_B);
		ID("bgezl", VM("000001:rs:00011:imm16"), "%s, %O", ADDR_TYPE_16, INSTR_TYPE_B or INSTR_TYPE_LIKELY);
		ID("bgezal", VM("000001:rs:10001:imm16"), "%s, %O", ADDR_TYPE_16, INSTR_TYPE_JAL);
		ID("bgezall", VM("000001:rs:10011:imm16"), "%s, %O", ADDR_TYPE_16, INSTR_TYPE_JAL or INSTR_TYPE_LIKELY);

		// Branch on Less Than Zero (And Link) (Likely).
		ID("bltz", VM("000001:rs:00000:imm16"), "%s, %O", ADDR_TYPE_16, INSTR_TYPE_B);
		ID("bltzl", VM("000001:rs:00010:imm16"), "%s, %O", ADDR_TYPE_16, INSTR_TYPE_B or INSTR_TYPE_LIKELY);
		ID("bltzal", VM("000001:rs:10000:imm16"), "%s, %O", ADDR_TYPE_16, INSTR_TYPE_JAL);
		ID("bltzall", VM("000001:rs:10010:imm16"), "%s, %O", ADDR_TYPE_16, INSTR_TYPE_JAL or INSTR_TYPE_LIKELY);

		// Branch on Less Or Equals than Zero (Likely).
		ID("blez", VM("000110:rs:00000:imm16"), "%s, %O", ADDR_TYPE_16, INSTR_TYPE_B);
		ID("blezl", VM("010110:rs:00000:imm16"), "%s, %O", ADDR_TYPE_16, INSTR_TYPE_B or INSTR_TYPE_LIKELY);

		// Branch on Great Than Zero (Likely).
		ID("bgtz", VM("000111:rs:00000:imm16"), "%s, %O", ADDR_TYPE_16, INSTR_TYPE_B);
		ID("bgtzl", VM("010111:rs:00000:imm16"), "%s, %O", ADDR_TYPE_16, INSTR_TYPE_B or INSTR_TYPE_LIKELY);

		// Branch on Not Equals (Likely).
		ID("bne", VM("000101:rs:rt:imm16"), "%s, %t, %O", ADDR_TYPE_16, INSTR_TYPE_B);
		ID("bnel", VM("010101:rs:rt:imm16"), "%s, %t, %O", ADDR_TYPE_16, INSTR_TYPE_B or INSTR_TYPE_LIKELY);
		...
		ID("bc1fl", VM("010001:01000:00010:imm16"), "%O", ADDR_TYPE_16, INSTR_TYPE_B or INSTR_TYPE_LIKELY);
		ID("bc1tl", VM("010001:01000:00011:imm16"), "%O", ADDR_TYPE_16, INSTR_TYPE_B or INSTR_TYPE_LIKELY);
		...
		ID("dbreak", VM("011100:00000:00000:00000:00000:111111"), "", ADDR_TYPE_NONE, INSTR_TYPE_PSP or INSTR_TYPE_BREAK);
		...
		ID("bvf", VM("010010:01:000:imm3:00:imm16"), "%Zc, %O", ADDR_TYPE_16, INSTR_TYPE_PSP or INSTR_TYPE_B);
		ID("bvt", VM("010010:01:000:imm3:01:imm16"), "%Zc, %O", ADDR_TYPE_16, INSTR_TYPE_PSP or INSTR_TYPE_B);
		ID("bvfl", VM("010010:01:000:imm3:10:imm16"), "%Zc, %O", ADDR_TYPE_16, INSTR_TYPE_PSP or INSTR_TYPE_B or INSTR_TYPE_LIKELY);
		ID("bvtl", VM("010010:01:000:imm3:11:imm16"), "%Zc, %O", ADDR_TYPE_16, INSTR_TYPE_PSP or INSTR_TYPE_B or INSTR_TYPE_LIKELY);
	}
}

data class InstructionType(
	val name: String,
	val vm: InstructionTable.ValueMask,
	val format: String,
	val addressType: Int,
...
private fun String.btquote() = when (this) {
	"break" -> "_$this"
	else -> this.replace('.', '_')
}
...
class DecodingTable {
	...
	companion object {
		...
		fun dumpImpl(instructions: ArrayList<InstructionType>): Unit {
			for (i in instructions) {
				println("open fun ${i.name.btquote()}(s: T): Unit = TODO()")
			}
		}
	}
	...
	private fun _createSwitch(writer: Indenter, instructions: List<InstructionType>, baseMask: Int = 0xFFFFFFFF.toInt(), level: Int = 0) {
		...
		writer.indent {
			for ((groupKey, group) in groups) {
				...
				writer.indent {
					if (group.size == 1) {
						writer.line("""return e.${group[0].name.btquote()}(s)""");
					} else {
