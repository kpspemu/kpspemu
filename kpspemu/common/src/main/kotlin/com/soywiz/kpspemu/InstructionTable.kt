...
//
	...
	val bgezall = ID("bgezall", VM("000001:rs:10011:imm16"), "%s, %O", ADDR_TYPE_16, INSTR_TYPE_LIKELY or INSTR_TYPE_JAL)
	...
	val bltzall = ID("bltzall", VM("000001:rs:10010:imm16"), "%s, %O", ADDR_TYPE_16, INSTR_TYPE_LIKELY or INSTR_TYPE_JAL)
	...
	val dbreak = ID("dbreak", VM("011100:00000:00000:00000:00000:111111"), "", ADDR_TYPE_NONE, INSTR_TYPE_PSP or INSTR_TYPE_BREAK)
	...
	val vpfxd = ID("vpfxd", VM("110111:10:------------:mskw:mskz:msky:mskx:satw:satz:saty:satx"), "[%vp4, %vp5, %vp6, %vp7]", ADDR_TYPE_NONE, INSTR_TYPE_PSP)
	val vpfxs = ID("vpfxs", VM("110111:00:----:negw:negz:negy:negx:cstw:cstz:csty:cstx:absw:absz:absy:absx:swzw:swzz:swzy:swzx"), "[%vp0, %vp1, %vp2, %vp3]", ADDR_TYPE_NONE, INSTR_TYPE_PSP)
	val vpfxt = ID("vpfxt", VM("110111:01:----:negw:negz:negy:negx:cstw:cstz:csty:cstx:absw:absz:absy:absx:swzw:swzz:swzy:swzx"), "[%vp0, %vp1, %vp2, %vp3]", ADDR_TYPE_NONE, INSTR_TYPE_PSP)
	...
	val vcmovf = ID("vcmovf", VM("110100:10:101:01:imm3:two:vs:one:vd"), "%zp, %yp, %v3", ADDR_TYPE_NONE, INSTR_TYPE_PSP)
	val vcmovt = ID("vcmovt", VM("110100:10:101:00:imm3:two:vs:one:vd"), "%zp, %yp, %v3", ADDR_TYPE_NONE, INSTR_TYPE_PSP)
	...
	val bvfl = ID("bvfl", VM("010010:01:000:imm3:10:imm16"), "%Zc, %O", ADDR_TYPE_16, INSTR_TYPE_PSP or INSTR_TYPE_B or INSTR_TYPE_LIKELY)
	val bvtl = ID("bvtl", VM("010010:01:000:imm3:11:imm16"), "%Zc, %O", ADDR_TYPE_16, INSTR_TYPE_PSP or INSTR_TYPE_B or INSTR_TYPE_LIKELY)
	val instructions = listOf(add, addu, addi, addiu, sub, subu, and, andi, nor, or, ori, xor, xori, sll, sllv, sra, srav, srl, srlv, rotr, rotrv, slt, slti, sltu, sltiu, lui, seb, seh, bitrev, max, min, div, divu, mult, multu, madd, maddu, msub, msubu, mfhi, mflo, mthi, mtlo, movz, movn, ext, ins, clz, clo, wsbh, wsbw, beq, beql, bgez, bgezl, bgezal, bgezall, bltz, bltzl, bltzal, bltzall, blez, blezl, bgtz, bgtzl, bne, bnel, j, jr, jalr, jal, bc1f, bc1t, bc1fl, bc1tl, lb, lh, lw, lwl, lwr, lbu, lhu, sb, sh, sw, swl, swr, ll, sc, lwc1, swc1, add_s, sub_s, mul_s, div_s, sqrt_s, abs_s, mov_s, neg_s, round_w_s, trunc_w_s, ceil_w_s, floor_w_s, cvt_s_w, cvt_w_s, mfc1, mtc1, cfc1, ctc1, c_f_s, c_un_s, c_eq_s, c_ueq_s, c_olt_s, c_ult_s, c_ole_s, c_ule_s, c_sf_s, c_ngle_s, c_seq_s, c_ngl_s, c_lt_s, c_nge_s, c_le_s, c_ngt_s, syscall, cache, sync, _break, dbreak, halt, dret, eret, mfic, mtic, mfdr, mtdr, cfc0, ctc0, mfc0, mtc0, mfv, mfvc, mtv, mtvc, lv_s, lv_q, lvl_q, lvr_q, sv_q, vdot, vscl, vsge, vslt, vrot, vzero, vone, vmov, vabs, vneg, vocp, vsgn, vrcp, vrsq, vsin, vcos, vexp2, vlog2, vsqrt, vasin, vnrcp, vnsin, vrexp2, vsat0, vsat1, vcst, vmmul, vhdp, vcrs_t, vcrsp_t, vi2c, vi2uc, vtfm2, vtfm3, vtfm4, vhtfm2, vhtfm3, vhtfm4, vsrt3, vfad, vmin, vmax, vadd, vsub, vdiv, vmul, vidt, vmidt, viim, vmmov, vmzero, vmone, vnop, vsync, vflush, vpfxd, vpfxs, vpfxt, vdet, vrnds, vrndi, vrndf1, vrndf2, vcmp, vcmovf, vcmovt, vavg, vf2id, vf2in, vf2iu, vf2iz, vi2f, vscmp, vmscl, vt4444_q, vt5551_q, vt5650_q, vmfvc, vmtvc, mfvme, mtvme, sv_s, vfim, svl_q, svr_q, vbfy1, vbfy2, vf2h, vh2f, vi2s, vi2us, vlgb, vqmul, vs2i, vc2i, vuc2i, vsbn, vsbz, vsocp, vsrt1, vsrt2, vsrt4, vus2i, vwbn, bvf, bvt, bvfl, bvtl)
...
private fun String.kescape() = when (this) {
	"break" -> "_$this"
	else -> this.replace('.', '_')
}
...
class DecodingTable {
	...
	companion object {
		fun createSwitch(instructions: List<InstructionType>): String {
			val writer = Indenter()
			val decodingTable = DecodingTable()
	...
	private fun _createSwitch(writer: Indenter, instructions: List<InstructionType>, baseMask: Int = 0xFFFFFFFF.toInt(), level: Int = 0) {
		...
		writer.indent {
			for ((groupKey, group) in groups) {
				writer.line("""${"0x%08X".format(groupKey)}.toInt() -> """)
				writer.indent {
					if (group.size == 1) {
						writer.line("""return e.${group[0].name.kescape()}(s)""")
					} else {
						this._createSwitch(writer, group, commonMask.inv(), level + 1)
					}
				}
			}
		}
