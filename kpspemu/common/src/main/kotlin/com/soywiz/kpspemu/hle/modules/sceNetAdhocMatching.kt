...
class sceNetAdhocMatching(emulator: Emulator) : SceModule(emulator, "sceNetAdhocMatching", 0x00010011, "pspnet_adhoc_matching.prx", "sceNetAdhocMatching_Library") {
	...
	override fun registerModule() {
		...
		registerFunctionRaw("sceNetAdhocMatchingGetPoolMaxAlloc", 0x40F8F435, since = 150) { sceNetAdhocMatchingGetPoolMaxAlloc(it) }
		registerFunctionRaw("sceNetAdhocMatchingSelectTarget", 0x5E3D4B79, since = 150) { sceNetAdhocMatchingSelectTarget(it) }
		registerFunctionRaw("sceNetAdhocMatchingTerm", 0x7945ECDA, since = 150) { sceNetAdhocMatchingTerm(it) }
		registerFunctionRaw("sceNetAdhocMatchingCancelTargetWithOpt", 0x8F58BEDF, since = 150) { sceNetAdhocMatchingCancelTargetWithOpt(it) }
		registerFunctionRaw("sceNetAdhocMatchingStart", 0x93EF3843, since = 150) { sceNetAdhocMatchingStart(it) }
		registerFunctionRaw("sceNetAdhocMatchingGetPoolStat", 0x9C5CFB7D, since = 150) { sceNetAdhocMatchingGetPoolStat(it) }
		registerFunctionRaw("sceNetAdhocMatchingSetHelloOpt", 0xB58E61B7, since = 150) { sceNetAdhocMatchingSetHelloOpt(it) }
		registerFunctionRaw("sceNetAdhocMatchingGetHelloOpt", 0xB5D96C2A, since = 150) { sceNetAdhocMatchingGetHelloOpt(it) }
		registerFunctionRaw("sceNetAdhocMatchingGetMembers", 0xC58BCD9E, since = 150) { sceNetAdhocMatchingGetMembers(it) }
		registerFunctionRaw("sceNetAdhocMatchingCreate", 0xCA5EDA6F, since = 150) { sceNetAdhocMatchingCreate(it) }
		registerFunctionRaw("sceNetAdhocMatchingCancelTarget", 0xEA3C6108, since = 150) { sceNetAdhocMatchingCancelTarget(it) }
		registerFunctionRaw("sceNetAdhocMatchingAbortSendData", 0xEC19337D, since = 150) { sceNetAdhocMatchingAbortSendData(it) }
