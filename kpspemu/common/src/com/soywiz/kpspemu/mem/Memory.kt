...
object MemoryInfo {
    ...
    val DUMMY: Memory by lazy { NewDataPointer(1024) }
}
...
fun Memory(): Memory = NewDataPointer(0x10000000)
...
fun Memory.sb(address: Int, value: Int) = run { alignedSetS8(index(address) ushr 0, value.toByte()) }
fun Memory.sh(address: Int, value: Int) = run { alignedSetS16(index(address) ushr 1, value.toShort()) }
fun Memory.sw(address: Int, value: Int) = run { alignedSetS32(index(address) ushr 2, value.toInt()) }

fun Memory.lb(address: Int) = alignedGetS8(index(address) ushr 0).toInt()
fun Memory.lh(address: Int) = alignedGetS16(index(address) ushr 1).toInt()
fun Memory.lw(address: Int): Int = alignedGetS32(index(address) ushr 2).toInt()
...
fun Memory.isValidAddress(address: Int): Boolean = address in MemoryInfo.MAINMEM || address in MemoryInfo.VIDEOMEM || address in MemoryInfo.SCRATCHPAD
...
fun Memory.copy(srcPos: Int, dstPos: Int, size: Int) = run {
    ...
    for (n in 0 until size) {
        alignedSetS8(dstOffset + n, alignedGetS8(srcOffset + n))
    }
}
